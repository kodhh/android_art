...
#endif
#ifdef ART_TARGET_ANDROID
// The MOVE ioctl is an optimization over the COPY ioctl, used for moving pages
// in the same address-space during concurrent compaction. It requires kernel
// >= 6.1 and may be blocked by seccomp filter (checked by MoveIoctlKernelCheck()).
...
void MarkCompact::CompactMovingSpace(uint8_t* page) {
  ...
  while (idx > black_dense_end_idx) {
    ...
    bool success = DoPageCompactionWithStateChange<kMode>(
        ...
        [&]() REQUIRES_SHARED(Locks::mutator_lock_) {
          CompactPage(first_obj, pre_compact_offset_moving_space_[idx], page, kMode == kUffdMode);
        });
